...
namespace Adventure
{
	public class Cave : EnemyCharacter
	{
		...
		public float TimeUntilNextGenerate { get; set; }
		...
		private void PickRandomFacingFor(PointF position)
		{
			...
			float maxDoorCanSee = 0;
			float preferredZRotation = 0;

			for (int i = 0; i < 8; i++) {
				double testZ = rnd.NextDouble() * (2 * Math.PI);
				var x = -Math.Sin (testZ) * 1024 + position.X;
				double y = Math.Cos (testZ) * 1024 + position.Y;

				Point pos2 = new Point ((int)x, (int)y);

				float dist = 0;
				if(scene != null)
					dist = scene.DistanceToWall(Point.Round(position), pos2);
		...
		public override void PerformDeath ()
		{
			...
			_smokeEmitter.RunAction (SKAction.Sequence (new SKAction[] {
				...
				// TODO RunBlock bad name for method https://trello.com/c/aeDO8V65
				SKAction.RunBlock (() => {
		...
		public void Generate()
		{
			...
			float offset = CollisionRadius * 0.75f;
			float rot = GraphicsUtilities.PalarAdjust(VirtualZRotation);
			PointF pos = new PointF((float)Math.Cos (rot) * offset, (float)Math.Sin (rot) * offset);
			character.Position = new PointF(pos.X + Position.X, pos.Y + Position.Y);
		...
		public static void LoadSharedAssetsOnce ()
		{
			...
			_sharedDamageAction = SKAction.Sequence (new SKAction[] {
				SKAction.ColorizeWithColor (UIColor.Red, 1, 0),
