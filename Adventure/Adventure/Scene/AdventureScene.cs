...
namespace Adventure
{
	...
	{
		private const int WorldTileDivisor = 32;	// number of tiles
		private const int WorldSize = 4096;			// pixel size of world (square)
		private const int WorldTileSize = WorldSize / WorldTileDivisor;

		private const int WorldCenter = 2048;

		private const int LevelMapSize = 256;		// pixel size of level map (square)
		...
		private Boss _levelBoss;                 // the big boss character
		...
		public AdventureScene (SizeF size)
			: base(size)
		{
		...
		public override void Initialize ()
		{
			...
			PointF startPosition = DefaultSpawnPoint;
		...
		private void AddSpawnPoints ()
		{
			...
			for (int y = 0; y < LevelMapSize; y++) {
				for (int x = 0; x < LevelMapSize; x++) {
					...
					PointF worldPoint = ConvertLevelMapPointToWorldPoint (location);
		...
		private void AddTrees()
		{
			for (int y = 0; y < LevelMapSize; y++) {
				for (int x = 0; x < LevelMapSize; x++) {
					...
					PointF treePos = ConvertLevelMapPointToWorldPoint (location);
			...
			for (int y = 0; y < LevelMapSize; y++) { // iterate in horizontal rows
				for (int x = 0; x < LevelMapSize; x++) {
					...
					while (horizontalDistanceFromLeft < LevelMapSize
						...
						nextSpot = _levelMap.QueryLevelMap (new Point (horizontalDistanceFromLeft, y));
					}
					...
					if (wallWidth > 8) {
						...
						while (verticalDistanceFromTop < LevelMapSize
							...
							nextSpot = _levelMap.QueryLevelMap (new Point (x + (wallWidth / 2), verticalDistanceFromTop));
						}
			...
			for (int x = 0; x < LevelMapSize; x++) { // iterate in vertical rows
				for (int y = 0; y < LevelMapSize; y++) {
					...
					while (verticalDistanceFromTop < LevelMapSize
						...
						verticalDistanceFromTop++;
						nextSpot = _levelMap.QueryLevelMap (new Point (x, verticalDistanceFromTop));
					}
					...
					if (wallHeight > 8) {
						...
						while (horizontalDistanceFromLeft < LevelMapSize
							&& nextSpot.Wall >= 200) {
							horizontalDistanceFromLeft++;
							nextSpot = _levelMap.QueryLevelMap (new Point (horizontalDistanceFromLeft, y + wallHeight / 2));
						}
		...
		private void AddCollisionWallAtWorldPoint(PointF worldPoint, float width, float height)
		{
			RectangleF rect = new RectangleF(0, 0, width, height);

			SKNode wallNode = SKNode.Create ();
			wallNode.Position = new PointF (worldPoint.X + rect.Size.Width * 0.5f, worldPoint.Y - rect.Size.Height * 0.5f);
		...
		public void StartLevel()
		{
			...
			PointF bossPosition = _levelBoss.Position; // set earlier from buildWorld in addSpawnPoints
			bossPosition.X += 128;
			bossPosition.Y += 512;
		...
		public override void DidSimulatePhysics ()
		{
			...
			foreach (Tree tree in _trees) {
				if (GraphicsUtilities.DistanceBetweenPoints (tree.Position, position) < 1024)
			...
			foreach (SKEmitterNode particles in _particleSystems) {
				bool particlesAreVisible = GraphicsUtilities.DistanceBetweenPoints(particles.Position, position) < 1024;
			...
			foreach (ParallaxSprite sprite in _parallaxSprites) {
				if (GraphicsUtilities.DistanceBetweenPoints(sprite.Position, position) >= 1024)
					continue;
		...
		public override float DistanceToWall (Point pos0, Point pos1)
		{
			PointF a = ConvertWorldPointToLevelMapPoint (pos0);
			PointF b = ConvertWorldPointToLevelMapPoint (pos1);

			float deltaX = b.X - a.X;
			float deltaY = b.Y - a.Y;
			float dist = GraphicsUtilities.DistanceBetweenPoints(a, b);
			float inc = 1f / dist;
			...
			for (float i = 0; i <= 1; i += inc) {
				...
				DataMap point = _levelMap.QueryLevelMap (Point.Round(p));
				if (point.Wall > 200) {
					PointF wpos2 = ConvertLevelMapPointToWorldPoint(Point.Round(p));
					return GraphicsUtilities.DistanceBetweenPoints(pos0, wpos2);
				}
			}
		...
		public override bool CanSee (PointF pos0, PointF pos1)
		{
			Point a = ConvertWorldPointToLevelMapPoint(pos0);
			Point b = ConvertWorldPointToLevelMapPoint(pos1);
		...
		private PointF ConvertLevelMapPointToWorldPoint(Point location)
		{
			...
			int x =  (location.X * LevelMapDivisor) - (WorldCenter + WorldTileSize / 2);
			int y = -((location.Y * LevelMapDivisor) - (WorldCenter + WorldTileSize / 2));

			return new PointF(x, y);
		}

		private Point ConvertWorldPointToLevelMapPoint(PointF location)
		{
